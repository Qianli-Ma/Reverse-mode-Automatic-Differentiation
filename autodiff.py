...
class MatMulOp(Op):
    ...
    def compute(self, node, input_vals):
        ...
        """TODO: Your code here"""
        assert len(input_vals) == 2
        in_A = input_vals[0] if not node.matmul_attr_trans_A else input_vals[0].T
        in_B = input_vals[1] if not node.matmul_attr_trans_B else input_vals[1].T
        return np.dot(in_A, in_B)
...
class Executor:
    ...
    def run(self, feed_dict):
        ...
        for node in topo_order:
            if not isinstance(node.op, PlaceholderOp) and node not in feed_dict:
        ...
        if not isinstance(node.op, PlaceholderOp):
            ...
            for i, input in enumerate(node.inputs):
                if input in node_to_output_grads_list:
                    ...
                    node_to_output_grads_list[input] = [grads[i]]
